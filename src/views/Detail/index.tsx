...
export default function Detail() {
  ...
  const fetchSingleProduct = async () => {
    try {
      const fetchingSingleProduct = await fetch(
        `http://localhost:4009/olx/api/singleproduct/${id}`
      );
      const productData = await fetchingSingleProduct.json();
      setSingleProduct(productData.productGet);
    } catch (error) {
      console.log("Error fetching product bhiya: ", error);
    }
  };
            ...
            <div key={id} className="flex gap-5 justify-between h-screen">
              ...
              <div className="border p-4 w-1/3 h-72 rounded">
                ...
                <div className="flex-cols justify-center text-center">
                  ...
                  <button className="border-2 text-[#002f34] font-bold border-[#002f34] p-2 rounded-sm w-full">
                    Add to Cart
                  </button>
                </div>
              </div>
